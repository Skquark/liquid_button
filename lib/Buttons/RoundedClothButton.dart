...
class RoundClothButton extends StatefulWidget {
  ...
  RoundClothButton({Key? key, required this.height, required this.width, required this.child, this.retainGradient = false, this.gap = 1, this.duration = const Duration(milliseconds: 500), this.gradientColor, required this.backgroundColor, this.expandFactor = 10.0}) : super(key: key) {
...
class _ClothButtonState extends State<RoundClothButton> with TickerProviderStateMixin {
  Offset? position;
    ...
    if (!kIsWeb)
      return GestureDetector(
        ...
        child: SizedBox(
          ...
          width: widget.width,
          child: CustomPaint(
            painter: RoundClothCustomPainter(gap: widget.gap, relativePosition: position, expandFactor: animation.value, backgroundColor: widget.backgroundColor, maxExpand: widget.expandFactor, retainGradient: widget.retainGradient, gradientColor: widget.gradientColor ?? widget.backgroundColor),
